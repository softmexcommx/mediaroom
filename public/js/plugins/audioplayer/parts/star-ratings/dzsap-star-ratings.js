...
class dzsapStarRatingsSource {
  ...
  init() {
    ...
    function mouse_starrating(e) {
      var $t = $(this); // console.log('mouse_starrating() ', e.type, $t);
      ...
      if (e.type === 'mouseleave') {
        var auxnr = Number(dzsapClass.cthis.find('.star-rating-con').eq(0).attr('data-initial-rating-index')) * 100; // console.log('dzsapClass.starrating_alreadyrated - ',dzsapClass.starrating_alreadyrated);
      ...
      if (e.type === 'mousemove') {
        ...
        var my = e.pageX - $t.offset().left; // console.log('Math.round(mx/ (_t.outerWidth()/5)) - ' , Math.round(mx/ (_t.outerWidth()/5)) );
      ...
      if (e.type === 'click') {
        console.log('dzsapClass.starrating_alreadyrated - ', dzsapClass.starrating_alreadyrated); //todo:remove
  ...
  if (selfClass.urlToAjaxHandler) {
    jQuery.ajax({
      ...
      success: function (response) {
        ...
        nrrates++;
        var percentFinalRating = ((nrrates - 1) * (percentSetRating * 5) + starrating_index) / nrrates; // console.log({percentSetRating, aux2: percentFinalRating});
      ...
      error: function (arg) {
        ...
        var nrrates = parseInt(selfClass.cthis.find('.counter-rates .the-number').html(), 10);
        nrrates++;
        var aux2 = ((nrrates - 1) * (widthStarRatingClip * 5) + starrating_index) / nrrates; // console.log({aux2});
